import React, { useState } from 'react';  // ✅ useState import karna tha
import { assets, cities } from '../assets/assets';
import { useAppContext } from '../context/AppContext';

const Hero = () => {
  const { navigate, getToken, axios, setSearchCities } = useAppContext();
  const [destination, setDestination] = useState("");  // ✅ useState define kiya
  
  const onSearch = async (e) => {
    e.preventDefault();
    navigate(`/rooms?destination=${destination}`);  // ✅ space hataya: `? destination` → `?destination`
    
    try {  // ✅ try-catch add kiya
      ...
      <form onSubmit={onSearch} className='bg-white text-gray-500 rounded-lg px-6 py-4 mt-8 flex flex-col md:flex-row max-md:items-start gap-4 max-md:mx-auto'>
        ...
        <div className='flex md:flex-col max-md:gap-2 max-md:items-center'>
          ...
          <input 
            ...
            placeholder="0" 
            defaultValue={1}  // ✅ default value add ki
          />
        </div>

        {/* Search Button */}
        <button 
          type="submit"  // ✅ type="submit" add kiya
